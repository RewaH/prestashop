...
public class PrestaShopAddressesActions extends UIInteractions {
	...
	public void givenUserCanCreateNewAddress() {
     ...
     //   AddressesObjects addObj= new AddressesObjects();
		...
		String filePath ="Utils/createNewAddressBody.xml";


		 XMLDataManager xmlDataManager = new XMLDataManager(filePath);
//		 //read the xml Body from xml file
//		 Document xmlDocument = xmlDataManager.readXMLFile();
//	      String requestBody=(XMLMarshaller.convertDocumentToString(xmlDocument));

		 
		 //Retrieve the Xml data with xpath
		 XMLMarshaller xmlMarshaller =new XMLMarshaller();
	        Node xmlNode = xmlDataManager.retrieveXMLDataAsXMLFormat("/prestashop");
	       String requestBody =(xmlMarshaller.nodeToString(xmlNode));
